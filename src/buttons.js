...
const MENU = {
  ...
  config: (cfg) => [
    ...
    [{ text: `🟡 Soft SL: -${cfg.softSlPct || 20}%`, callback_data: 'cfg_input_softSlPct' },
     { text: `⏱ Wait: ${cfg.softSlWaitSec || 30}s`, callback_data: 'cfg_input_softSlWaitSec' }],
    [{ text: `🛑 Hard SL: -${cfg.hardSlPct || cfg.slPct || 25}%`, callback_data: 'cfg_input_hardSlPct' },
...
async function sendMainMenu(chatId) {
  ...
  const lines = [
    ...
    `💰 Buy: ${cfg.buyAmountSol ?? 0.015} SOL | 🟡 Soft SL: -${cfg.softSlPct || 20}%/${cfg.softSlWaitSec || 30}s | 🛑 Hard SL: -${cfg.hardSlPct || cfg.slPct || 25}%`,
 ...
 try {
  ...
  for (const pos of open) {
    ...
    const lines = [
      ...
      `🟡 Soft SL: -${cfg.softSlPct || 20}%/${cfg.softSlWaitSec || 30}s | 🛑 Hard SL: -${cfg.hardSlPct || cfg.slPct || 25}%`,
      trailLine,
      psLine,
...
async function sendConfigMenu(chatId) {
  try {
    ...
    const lines = [
      ...
      `🟡 Soft SL: -${cfg.softSlPct || 20}%/${cfg.softSlWaitSec || 30}s`,
      `🛑 Hard SL: -${cfg.hardSlPct || cfg.slPct || 25}%`,
